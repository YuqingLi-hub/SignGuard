...
def all_worker(model, train_loader, optimizer, args, water=False,malicious=True):
    ...
    logger.info(f'Client Received parameters {global_param[masks[0]:masks[1]][:10]}')
    ...
    if water:
        ...
        with torch.no_grad():
            ...
            for p in model.parameters():
                ...
                p = g
                start += numel
        logger.info(f'Updated recovered params {tools.get_parameter_values(model)[masks[0]:masks[1]][:10]}')
    # ------------------ Training step ------------------
    model.train()
    ...
    # IMPORTANT: zero local grads
    for p in model.parameters():
        if p.grad is not None:
            p.grad.zero_()

    # if recovered_grads is not None:
    #     # ✅ Use recovered gradients instead of backward()
    # tools.update_gradient(global_param, model)
    # optimizer.step()
    ...
    logger.info(f'Updated grads (first 10): {updated_grads[masks[0]:masks[1]][:10]}')
