...
class Agent():
    def __init__(self, id, args, train_dataset=None, data_idxs=None, mask=None, backdoor_train_dataset=None):
        ...
        if self.args.dataset != "tinyimagenet":
            ...
            if self.id < args.num_byzs and self.args.attack != 'non' and self.args.dataset != 'sen140':
            ...
            elif self.id < args.num_byzs and self.args.attack != 'non' and self.args.dataset == 'sen140':
                self.clean_backup_dataset = copy.deepcopy(train_dataset)
                self.data_idxs = data_idxs
    ...
    def local_train(self, global_model, criterion, round=None, neurotoxin_mask=None):
        ...
        for local_epoch in range(self.args.local_ep):
            ...
            for i, (inputs, labels) in enumerate(self.train_loader):
                ...
                if self.args.attack == "neurotoxin" and len(neurotoxin_mask) and self.id < self.args.num_byzs:
                    for name, param in global_model.named_parameters():
                        param.grad.data = neurotoxin_mask[name].to(self.args.device) * param.grad.data
                if self.args.attack == "r_neurotoxin" and len(neurotoxin_mask) and self.id < self.args.num_byzs:
                    for name, param in global_model.named_parameters():
                ...
                if self.args.attack == 'pgd' and self.id < self.args.num_byzs and (i == len(self.train_loader) - 1):
